...
class VUOS_ObjectiveHandler : EventHandler abstract
{
    // ====================================================================
    // OBJECTIVE STORAGE
    // Stored on the EventHandler instance. Serializes with save/load and
    ...
    static void AddObjective(String desc, name target = '', int count = 1, int objType = 0, bool hidden = false, bool persist = true, bool inverse = false, int timeLimit = 0, bool isPrimary = true, int requiredToComplete = -1, int minSkillLevel = 0, int maxSkillLevel = 4, double wpX = 0, double wpY = 0, double wpZ = 0)
    {
        ...
        obj.objectiveType = objType;             // 0 = kill, 1 = destroy, 2 = collect (inventory), 3 = custom
        ...
        obj.isAutoGenerated = false;              // Default to manual; auto-objectives handler sets this to true

        // Populate multi-waypoint arrays for consistency with multi-waypoint system
        if (obj.hasWaypoint)
        {
            obj.PushWaypointPosition(wpX, wpY, wpZ);
        }
    ...
    // Convenience method: Add a PRIMARY COLLECT objective (objType=2, auto-completes when player picks up targetClass)
    // Usage: AddPrimaryCollectObjective("Find the red key", 'RedCard');
    static void AddPrimaryCollectObjective(String desc, name target, int count = 1, bool hidden = false, bool persist = true, int requiredToComplete = -1, int minSkillLevel = 0, int maxSkillLevel = 4, double wpX = 0, double wpY = 0, double wpZ = 0)
    {
        AddObjective(desc, target, count, 2, hidden, persist, false, 0, true, requiredToComplete, minSkillLevel, maxSkillLevel, wpX, wpY, wpZ);
    }

    // Convenience method: Add a SECONDARY COLLECT objective (objType=2, auto-completes when player picks up targetClass)
    // Usage: AddSecondaryCollectObjective("Find a health pack", 'Medikit');
    static void AddSecondaryCollectObjective(String desc, name target, int count = 1, bool hidden = false, bool persist = true, int requiredToComplete = -1, int minSkillLevel = 0, int maxSkillLevel = 4, double wpX = 0, double wpY = 0, double wpZ = 0)
    {
        AddObjective(desc, target, count, 2, hidden, persist, false, 0, false, requiredToComplete, minSkillLevel, maxSkillLevel, wpX, wpY, wpZ);
    }
    ...
    static void SetWaypoint(String desc, double x, double y, double z)
    {
        ...
        // Also populate multi-waypoint arrays for consistency with multi-waypoint system
        obj.ClearWaypointPositions();
        obj.trackedActors.Clear();
        if (obj.hasWaypoint)
        {
            obj.PushWaypointPosition(x, y, z);
        }
    ...
    // Set multiple waypoint positions for an existing objective (for multi-position objectives)
    // Replaces any existing waypoint data with the provided parallel coordinate arrays
    static void SetWaypointMulti(String desc, Array<double> posX, Array<double> posY, Array<double> posZ)
    {
        let obj = FindByDescription(desc);
        if (!obj) return;
        ...
        {
            obj.PushWaypointPosition(posX[i], posY[i], posZ[i]);
        }

        // Set primary waypointPos from first position
        if (posX.Size() > 0)
        {
            obj.waypointPos = (posX[0], posY[0], posZ[0]);
            obj.hasWaypoint = true;
        }
        else
        {
    ...
    // Set multiple waypoint positions with associated actor references (for cleanup on death/pickup)
    static void SetWaypointMultiWithActors(String desc, Array<double> posX, Array<double> posY, Array<double> posZ, Array<Actor> actors)
    {
        let obj = FindByDescription(desc);
        if (!obj) return;
        ...
        {
            obj.trackedActors.Push(actors[i]);
        }
    ...
    static void ClearWaypoint(String desc)
    {
        ...
        DestroyMarker(obj);
        // Destroy any multi-position markers
        for (int i = 0; i < obj.markerActors.Size(); i++)
        {
            if (obj.markerActors[i]) obj.markerActors[i].Destroy();
        }
        obj.markerActors.Clear();
        obj.ClearWaypointPositions();
        obj.trackedActors.Clear();
        obj.waypointPos = (0, 0, 0);
        obj.hasWaypoint = false;
    ...
    // Spawn additional automap markers for multi-position waypoints
    // Called after SpawnMarker for objectives with multiple positions
    static void SpawnMultiMarkers(VUOS_ObjectiveData obj, int markerNumber = 0)
    {
        if (!obj || obj.GetWaypointPositionCount() <= 1) return;
        if (!(obj.mapName ~== level.MapName)) return;
        if (!GetCVarBool('obj_automap_markers', GetFirstPlayer(), true)) return;
        ...
        // Determine frame for the markers
        int frameIdx;
        if (obj.isCompleted)
            frameIdx = 3; // D = completed
        else if (obj.hasFailed)
            frameIdx = 4; // E = failed
        else if (!obj.isTracked)
            frameIdx = 2; // Grey for untracked
        else
            frameIdx = obj.isPrimary ? 0 : 1;

        // Spawn a marker at each position (skip index 0, that's handled by the primary markerActor)
        for (int i = 1; i < obj.GetWaypointPositionCount(); i++)
        {
            let marker = Actor.Spawn("VUOS_ObjectiveMarker", obj.GetWaypointPosition(i));
            if (marker)
            {
                marker.ChangeStatNum(Actor.STAT_MAPMARKER);
                marker.args[2] = 1;
                ...
                else if (markerNumber >= 1 && markerNumber <= 9)
                {
                    if      (markerNumber == 1) marker.sprite = Actor.GetSpriteIndex('VOM1');
                    else if (markerNumber == 2) marker.sprite = Actor.GetSpriteIndex('VOM2');
                    else if (markerNumber == 3) marker.sprite = Actor.GetSpriteIndex('VOM3');
    ...
    // Destroy all automap markers for an objective (including multi-position markers)
    static void DestroyAllMarkers(VUOS_ObjectiveData obj)
    {
        if (!obj) return;
        DestroyMarker(obj);
        for (int i = 0; i < obj.markerActors.Size(); i++)
    ...
    static void RefreshAllMarkers()
    {
        ...
        // Check automap multi-mode CVAR (0 = closest, 1 = all)
        int automapMultiMode = GetCVarInt('vuos_automap_multi_mode', fp, 0);

        // Destroy markers for objectives that should NOT have markers
        for (int i = 0; i < handler.objectives.Size(); i++)
        {
            let obj = handler.objectives[i];
            bool hasAnyMarker = (obj.markerActor != null) || (obj.markerActors.Size() > 0);
        ...
        for (int i = 0; i < activeWaypoints.Size(); i++)
        {
            ...
            // For multi-position objectives in "closest" mode, update primary waypointPos
            // to the closest position before spawning the marker
            if (obj.GetWaypointPositionCount() > 1 && automapMultiMode == 0)
            {
                // Find closest position to first active player
                PlayerInfo pp = GetFirstPlayer();
                if (pp && pp.mo)
                {
                    int closest = obj.GetClosestPositionIndex(pp.mo.pos);
                    if (closest >= 0)
                        obj.waypointPos = obj.GetWaypointPosition(closest);
                }
            }

            SpawnMarker(obj, number);

            // In "all" mode, spawn additional markers for remaining positions
            if (automapMultiMode == 1 && obj.GetWaypointPositionCount() > 1)
            {
                SpawnMultiMarkers(obj, number);
            }
        }
    ...
    static void ClearAll()
    {
        ...
        // Destroy all automap markers before clearing (including multi-position markers)
        for (int i = 0; i < handler.objectives.Size(); i++)
        {
            DestroyMarker(handler.objectives[i]);
            let obj = handler.objectives[i];
            for (int m = 0; m < obj.markerActors.Size(); m++)
            {
                if (obj.markerActors[m]) obj.markerActors[m].Destroy();
            }
        }
    ...
    override void WorldTick()
    {
        ...
        for (int i = 0; i < objectives.Size(); i++)
        {
            ...
            // For multi-position objectives, cache distance to the closest position
            if (updateDistance && obj.hasWaypoint && !obj.isCompleted && !obj.hasFailed)
            {
                for (int p = 0; p < MAXPLAYERS; p++)
                {
                    ...
                    if (obj.GetWaypointPositionCount() > 1)
                    {
                        // Multi-position: find closest
                        double closestDist = level.Vec3Diff(playerPos, obj.GetWaypointPosition(0)).Length();
                        for (int wp = 1; wp < obj.GetWaypointPositionCount(); wp++)
                        {
                            double dist = level.Vec3Diff(playerPos, obj.GetWaypointPosition(wp)).Length();
                            if (dist < closestDist) closestDist = dist;
                        }
                        obj.SetDistance(p, int(closestDist));
                    }
                    else
                    {
                        // Single position: use primary waypointPos
        ...
        {
            let obj = objectives[i];
            if (obj.objectiveType != 2) continue; // 2 = TYPE_COLLECT
            if (obj.isCompleted || obj.hasFailed) continue;
            if (obj.targetClass == '') continue;
            if (!obj.IsVisibleForCurrentMap(skill)) continue;

            for (int p = 0; p < MAXPLAYERS; p++)
            {
                if (!playeringame[p] || !players[p].mo) continue;
                if (players[p].mo.CountInv(obj.targetClass) > 0)
                {
                    if (IsDebugEnabled()) Console.Printf("DEBUG [VUOS_ObjectiveHandler]: Player %d has %s in inventory, completing '%s'",
                        p, obj.targetClass, obj.objectiveDescription);
                    CompleteObjective(obj);
                    break;
                }
            }
